...
export function load():Progress{try{const old=JSON.parse(localStorage.getItem(KEY)||localStorage.getItem("ebtt-ifmt-progress-v1")||"{}");return{...initial,...old,profile:{...initial.profile,...old.profile},settings:{...initial.settings,...old.settings,weights:{...initial.settings.weights,...old.settings?.weights}},generatedQuestions:old.generatedQuestions||[],attemptHistory:old.attemptHistory||[],studyHistory:old.studyHistory||[]}}catch{return initial}}
